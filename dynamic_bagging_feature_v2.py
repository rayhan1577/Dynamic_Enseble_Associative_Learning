...
def test_uci():
    ...
    #sys.stdout = file
    data=["heart"]
    #data=["zoo", "pima","glass","pageBlocks","hepati","wine","anneal","horse","ionosphere","adult"]
    for ov in [15]:
        print("\n\nwith",ov/25,"overlap")
        ...
        for dataset_name in data:
            try:
                ...
                for i in range(100):
                    ...
                    sample, sample_test = subsample(X_train, X_test,ov)
                    #print(type(sample))
                ...
                end_time = time.time()
                print( end_time - start_time,  memory)
                print("base learner",X.shape[1],count_base_learner(X.shape[1]))
...
def subsample(dataset, test_dataset,ov):
    ...
    if(len(collect)==0):
            ...
            correlation_df[0]=np.nan_to_num(correlation_df[0])
            #print(correlation_df[0])
            correlations=np.copy(correlation_df[0])
            correlations.sort()
            ...
            for i in range(len(correlations)):
                if(len(list_features)==feat):
                    #print(list_features)
                    break
                for j in range(len(correlation_df[0])):
                    if(round(correlations[i],4)==round(correlation_df[0][j],4)):
            ...
            for i in list_features:
                sample[sample.shape] = df[i]
                test_sample[test_sample.shape] = df2[i]
            collect.append(list_features)
