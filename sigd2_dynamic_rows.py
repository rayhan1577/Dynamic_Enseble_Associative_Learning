...
def test_uci():
    ...
    #"Iris","Breast","Heart","Wine","Pima",
    #dataset=["Led7","glass","Anneal","Hepati","Horse","Adult","pageblocks","Zoo","Flare"]
    dataset=["Anneal","Hepati","Horse","Adult"]
    for dataset_name in dataset:
        try:
            ...
            end_time = time.time()
            print("{:.4f}".format(accuracy_score(y_test, final_prediction)), end=" ")
            print(generated_counter, final_counter, "{:.2f}".format(time.time() - start_time), no_of_predictor)
            #print("required time:", end_time - start_time)
        except:
            print("Error with dataset", dataset_name)
...
def subsample(dataset, y):
    ...
    while(run_loop and tracker<10):
        ...
        c2 = [i for i in all_count if i > overlap]    
        if(len(c2)>0):
            tracker+=1
        else:
            break
